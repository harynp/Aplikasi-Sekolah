...
router.get('/', (req,res) => {
  ...
  .then(teachers=>{
    ...
    Promise.all(promise)
    .then(result =>{
      res.render('teachers',{dataTeachers:result,session:req.session})
    })
  })
})
...
router.get('/add',(req,res)=>{
  Model.Teacher.findAll()
  .then(teachers=>{
    res.render('add_teachers',{dataTeachers:teachers, dataError: null,session:req.session})
  })
  .catch(err=>{
    res.send(err)
  })
...
router.get('/edit/:id',(req,res)=>{
  ...
  .then(data=>{
    ...
    .then(subject=>{
      res.render('edit_teachers', {dataTeachers:data, dataSubjects:subject,session:req.session})
    })
  })
  .catch(err=>{
    res.send(err)
